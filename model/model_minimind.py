...
class MiniMindConfig(PretrainedConfig):
    ...
    def __init__(self, hidden_size=768, num_hidden_layers=8, use_moe=False, **kwargs):
        ...
        self.hidden_size = hidden_size  # 隐藏层维度：每个token的向量长度，决定模型宽度
        self.num_hidden_layers = num_hidden_layers  # Transformer层数，决定模型深度
        self.use_moe = use_moe  # 是否启用MoE混合专家（False时下方MoE参数全部失效）
...
class RMSNorm(torch.nn.Module):
    def __init__(self, dim: int, eps: float = 1e-5):
        super().__init__()
        self.eps = eps  # 防除以 0 的极小安全值 (epsilon)
        self.weight = nn.Parameter(
            torch.ones(dim)
        )  # 可学习的缩放权重，初始全为 1.0 (恒等缩放)

    def norm(self, x):
        # 1. x.pow(2)               : 每个元素求平方 (量化特征能量)
        # 2. .mean(-1, keepdim=True): 沿最后一个维度求均值，保留维度为 1 方便广播
        # 3. + self.eps             : 加上微小值防除以 0 导致溢出 (NaN)
        # 4. torch.rsqrt(...)       : 硬件级开方取倒数 (1 / RMS)
        # 5. x * ...                : 原数据乘以倒数，完成均方根尺度归一化
        return x * torch.rsqrt(x.pow(2).mean(-1, keepdim=True) + self.eps)

    def forward(self, x):
        # 1. x.float()         : 临时升到 float32，防止半精度 (fp16/bf16) 平方时数值溢出
        # 2. self.norm(...)    : 执行高精度均方根归一化
        # 3. self.weight * ... : 乘上可学习权重，允许模型自适应调节各维度重要性
        # 4. .type_as(x)       : 还原为输入原本的数据类型 (如 bf16)，节约显存
