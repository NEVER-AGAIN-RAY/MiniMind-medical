...
本实验的规模曲线全程使用 lr 2e-4，而这个学习率现在已知偏低。**「9600 条饱和」应当视为
「在 lr 2e-4 下成立」，而不是这个任务的固有性质。** 要确认饱和点的真实位置，需要在
lr 8e-4 下重跑整条曲线（6 个点，约 15 分钟）。同样的重跑也适用于情感实验的 2000 条饱和点。
...
| [`saturation.md`](saturation.md) | 自动生成的规模曲线与对照分析 |
