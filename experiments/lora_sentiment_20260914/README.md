...
相邻点差几个百分点说明不了问题——n=400 时抽样噪声本身就有几个百分点。各点评测的是
同一批 400 道题，所以用 McNemar 配对检验（见 [saturation.md](saturation.md)，
由 [`analyze_saturation.py`](analyze_saturation.py) 生成）。

**提升在 2000 条处止步。** 之后把数据扩到语料上限 3672 条（近一倍）、再把训练轮数从
3 提到 9，都无法通过显著性检验。9 epoch 那次的 val_loss 在 step 400 触底 0.0827 后
回升 26%，是明确的过拟合。
...
- **预测分布未塌缩**：两类各约 200 题，两类 F1 相当
- **候选打分与生成式准确率几乎一致**：模型的判断与它实际吐出的字一致
