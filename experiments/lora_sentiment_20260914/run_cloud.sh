...
#
# 用法：
#   bash experiments/lora_sentiment_20260914/run_cloud.sh                # 全流程
#   bash experiments/lora_sentiment_20260914/run_cloud.sh --skip-smoke   # 跳过 Step 2
#   bash experiments/lora_sentiment_20260914/run_cloud.sh --skip-scale   # 跳过 Step 5
#   bash experiments/lora_sentiment_20260914/run_cloud.sh --step 4       # 只跑 Step 4
#   bash experiments/lora_sentiment_20260914/run_cloud.sh --from-step 3  # 从 Step 3 开始
#   bash experiments/lora_sentiment_20260914/run_cloud.sh --force        # 已有产物也重跑
#
...
ROOT = Path.cwd()
EXP = ROOT / "experiments" / "lora_sentiment_20260914"
ok = True
...
if should_run 2 && [ "${SKIP_SMOKE}" != true ]; then
    banner 2 "冒烟（40 条训练 + 20 条评测，仅验证流程）"
    SMOKE_DIR="${RUNS_DIR}/smoke"
    if done_already "${SMOKE_DIR}/eval_smoke.json"; then
        echo "⏭  已有冒烟产物，跳过（--force 可重跑）"
    else
        "${PYTHON}" "${EXP_REL}/train_sentiment_lora.py" --smoke --overwrite --run-dir "${SMOKE_DIR}"
        "${PYTHON}" "${EXP_REL}/eval_sentiment.py" --split smoke \
...
if should_run 3; then
    ...
    if done_already "${FORMAL_DIR}/train_summary.json"; then
        ...
        "${PYTHON}" "${EXP_REL}/train_sentiment_lora.py" --run-dir "${FORMAL_DIR}" ${EXTRA}
        echo "✅ Step 3 完成"
    fi
fi
