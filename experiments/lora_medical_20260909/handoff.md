...
>
> **2026-09-10 更新**：云端首跑已完成（详见 `eval_results/compare_report.md`）。首跑数据表明生成式 MCQ 评测与 64M 模型量级不匹配（~90% 回答无法解析），下一代评测标准已设计定稿 → **[eval_v3_design.md](eval_v3_design.md)**（MCQ 改 logprob 评分、新增通用 val_loss 防遗忘监控、复读率等生成健康度指标）。v2 流程与产物仍为当前有效基线。
